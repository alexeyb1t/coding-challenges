 ...
 * the described process so it won't be accepted as a result. The property (P) doesn't give unicity by itself.
 ...
 * */
export class G964 {
    public static arrange(str: string): string {
        if (str.length === 0) {
            return "";
        }
        const lengths = str.split(" ").map(subStr => subStr.length);
        // TODO: finish it
    }
}

console.log(G964.arrange(""), ""); // 0
console.log(G964.arrange("who hit retaining The That a we taken"), "who RETAINING hit THAT a THE we TAKEN"); // 3
console.log(G964.arrange("on I came up were so grandmothers"), "i CAME on WERE up GRANDMOTHERS so"); // 4
console.log(G964.arrange("way the my wall them him"), "way THE my WALL him THEM"); // 1
